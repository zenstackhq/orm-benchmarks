...
export function compareResults(results: {
    ...
    zenstackResults: MultipleBenchmarkRunResults;
}) {
    const orms: ORM[] = ['prisma', /*'drizzle', 'typeorm',*/ 'zenstack'];
    const { prismaResults, typeormResults, drizzleResults, zenstackResults } =
        results;
    ...
    for (
        ...
        queriesInRun.forEach((queryResult, queryIndex) => {
            ...
            const idSets: { [key in ORM]: Set<any> } = {
                prisma: extractIds(
                    prismaResults[iterationIndex][queryIndex].data
                ),
                drizzle: extractIds(
                    drizzleResults[iterationIndex][queryIndex].data
                ),
                typeorm: extractIds(
                    typeormResults[iterationIndex][queryIndex].data
                ),
                zenstack: extractIds(
                    zenstackResults[iterationIndex][queryIndex].data
                ),
            };
            ...
            if (!isDataConsistent) {
                console.log(
                    `Data is DIFFERENT for query "${query}" in run ${
                        iterationIndex + 1
                    }:`
                );
                orms.forEach((orm) => {
                ...
                console.log(
                    `Data is SAME for query "${query}" in run ${
                        iterationIndex + 1
                    }:`
                );
            }
